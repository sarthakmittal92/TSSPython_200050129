...
def GetMomentumBasedPriority(PartitionedDataFrameList, DateToIndex ,today):
  ...
  for i in range(DateToIndex[temp] + 1, DateToIndex[today]):
...
class PortFolio:
  ...
  def CalculateNetWorth(self):
    return self.balance + np.sum(self.numStocks * self.prices) + B * (1 - R)
  ...
  def RebalancePortFolio(self, newWeights):
    ...
    if balanceCopy + np.sum(self.prices * newStocks) + B * (1 - R) >= self.CalculateNetWorth():
...
Data = GetData("/home/sarthak/IITB/GitHub/TSSPython_200050129/Week-2/DATA.csv")
